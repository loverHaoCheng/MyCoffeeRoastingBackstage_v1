...
- 修复正式端版本提示在刷新或后台恢复后重复出现：已确认的构建版本现在持久化到浏览器本地存储。
- 修复启动清理逻辑误删版本记录，避免版本提示在每次进入后台后重新出现。
- 增加后台快照恢复时的版本校验，避免移动浏览器恢复旧页面构建。
- 新增全局 EasyBake 开屏动画：首次打开、刷新和应用更新后的主动刷新会在认证及 `/beans` 首屏查询完成后执行撕裂转场。
