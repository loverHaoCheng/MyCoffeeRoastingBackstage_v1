...
if [[ -f "${LOCAL_ENV_FILE}" ]]; then
  while IFS= read -r line || [[ -n "${line}" ]]; do
    ...
    value="${line#*=}"
      case "${key}" in
      APP_URL|EASYBAKE_APP_ENV|VITE_EASYBAKE_APP_ENV|REMOTE_TARGET|BFF_REMOTE_TARGET|BFF_SERVICE_NAME|BFF_LOCAL_PORT|FRONTEND_RELEASES_PATH|FRONTEND_CURRENT_LINK|FRONTEND_DEPLOY_LOCK_PATH|DEPLOY_ENVIRONMENT_NAME|DEPLOY_HTTP_USER|DEPLOY_HTTP_PASSWORD|POCKETBASE_DEPLOY_ENABLED|POCKETBASE_SERVICE_NAME|POCKETBASE_REMOTE_DIR|POCKETBASE_EXTENSION_REMOTE_DIR|POCKETBASE_HTTP_URL|POCKETBASE_RELEASE_ID)
        export "${key}=${value}"
        ;;
      *)
...
export POCKETBASE_SERVICE_NAME="${POCKETBASE_SERVICE_NAME:-pocketbase-staging}"
export POCKETBASE_REMOTE_DIR="${POCKETBASE_REMOTE_DIR:-/opt/pocketbase-staging}"
export POCKETBASE_EXTENSION_REMOTE_DIR="${POCKETBASE_EXTENSION_REMOTE_DIR:-/opt/easybake-pocketbase-extension}"
export POCKETBASE_HTTP_URL="${POCKETBASE_HTTP_URL:-http://127.0.0.1:8091}"
