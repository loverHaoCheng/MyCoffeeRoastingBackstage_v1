...
前端版本目录默认最多保留 `5` 个，其中当前版本（`/var/www/easybake`）与上一版本（`/var/www/easybake.previous`）始终受保护；其余版本按修改时间从新到旧保留，超出的目录在发布验收成功后通过 `sudo` 清理。需要调整数量时可执行 `FRONTEND_RELEASES_TO_KEEP=8 ./deploy.sh`；该值不得小于 `2`。
...
- 每条生豆必须选择成本模板。当前库存先按“剩余生豆重量 ÷ 模板生豆重量”计算可烘焙锅数，再按模板的脱水率和出售单份重量计算计划份数；收入扣除包装、能耗和其他费用，利润再扣除生豆成本。烘焙记录的已售份数不得超过该锅计划份数，关联邮费可多选烘焙记录并平均分摊。
