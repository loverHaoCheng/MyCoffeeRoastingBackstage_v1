...
export function useArchiveRoastingMachine() { const queryClient = useQueryClient(); return useMutation({ mutationFn: (machineId: string) => roasterMachineService.archive(machineId), onSuccess: () => queryClient.invalidateQueries({ queryKey: roasterMachineQueryKeys.machines() }) }); }
