...
未验证账号清理使用部署后的 BFF 内部任务，由 systemd 在每日北京时间 `03:00` 分别调用 `127.0.0.1:3001` 和 `127.0.0.1:3002`。任务只删除注册超过 24 小时且仍未验证邮箱的 `users` 记录；定时器配置在功能部署验收后创建，不与 PocketBase 数据目录或普通前端发布目录耦合。
