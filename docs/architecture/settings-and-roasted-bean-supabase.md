...
- 熟豆 Supabase 只保留两条链路：应用认证完成后的后台配置加载/探活（设置页复用状态），以及烘焙历史创建成功后的熟豆镜像写入。
- 熟豆连接配置在单次前端运行时内会保留在 Zustand / runtime memory 中。`AppRoastedBeanConnectionProbe` 在认证完成后加载配置并探活一次，避免用户在页面间来回切换时重复回拉与重复探活；刷新页面或重新启动 PWA 后会重新执行一次校验。
